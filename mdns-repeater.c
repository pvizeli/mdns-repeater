...
static int create_recv_sock()
{
    ...
    if ((r = bind(sd, (struct sockaddr *)&serveraddr, sizeof(serveraddr))) <
        0) {
        log_message(LOG_ERR, "recv bind(): %s", strerror(errno));
    }

    // enable loopback in case someone else needs the data
    if ((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_LOOP, &on, sizeof(on))) <
        0) {
...
static int create_send_sock(int recv_sockfd, const char *ifname,
                            struct if_sock *sockdata)
{
    ...
    if ((r = setsockopt(sd, SOL_SOCKET, SO_BINDTODEVICE, &ifr,
                        sizeof(struct ifreq))) < 0) {
        log_message(LOG_ERR, "send setsockopt(SO_BINDTODEVICE): %s",
                    strerror(errno));
        return r;
    }
#endif
    ...
    if ((r = setsockopt(recv_sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq,
                        sizeof(mreq))) < 0) {
    ...
    if ((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_LOOP, &on, sizeof(on))) <
        0) {
        log_message(LOG_ERR, "send setsockopt(IP_MULTICAST_LOOP): %s",
                    strerror(errno));
        return r;
    }
...
static ssize_t send_packet(int fd, const void *data, size_t len)
{
    ...
    return sendto(fd, data, len, 0, (struct sockaddr *)&toaddr,
                  sizeof(struct sockaddr_in));
}

static void mdns_repeater_shutdown(int sig) { shutdown_flag = 1; }
...
static void show_help(const char *progname)
{
    ...
    fprintf(stderr, "Copyright (C) 2020 Pascal Vizeli\n\n");
...
static int parse_opts(int argc, char *argv[])
{
    int c;
    int help = 0;
    while ((c = getopt(argc, argv, "hfl:p:")) != -1) {
        switch (c) {
        case 'h':
            help = 1;
            break;
...
int main(int argc, char *argv[])
{
    ...
    if (!socks) {
        log_message(
            LOG_ERR,
            "cannot allocate memory for requested number of interfaces");
        exit(3);
    }
    ...
    for (i = optind; i < argc; i++) {
        int sockfd =
            create_send_sock(server_sockfd, argv[i], &socks[num_socks]);
        if (sockfd < 0) {
            log_message(LOG_ERR, "unable to create socket for interface %s",
                        argv[i]);
            r = 1;
            goto end_main;
        }
    ...
    if (pkt_data == NULL) {
        log_message(LOG_ERR, "cannot malloc() packet buffer: %s",
                    strerror(errno));
        r = 1;
        goto end_main;
    }
    ...
    while (!shutdown_flag) {
        ...
        if (FD_ISSET(server_sockfd, &sockfd_set)) {
            ...
            for (j = 0; j < num_socks; j++) {
                ...
                // repeat data
                ssize_t sentsize =
                    send_packet(socks[j].sockfd, pkt_data, (size_t)recvsize);
                if (sentsize != recvsize) {
                    if (sentsize < 0)
                        ...
                        log_message(
                            LOG_ERR,
                            "send_packet size differs: sent=%ld actual=%ld",
                            recvsize, sentsize);
                }
            }
        }
